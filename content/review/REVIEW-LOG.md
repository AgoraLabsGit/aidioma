...
| `a1-04-donde-esta` | 2026-07-28 | L1-PASS | PENDING | PENDING | PENDING | 1 | Where things and people are. Original content-pipeline draft: 15 vocab, 20 sentences, 6 passage segments, 4 quick checks, no reference card (the compact *estar*/*hay* explanation is sufficient). Both-direction accept sets reviewed during author pre-flight; all sentence `acceptedEs` arrays have 3–4 alternates and every sentence has reviewed `acceptedEn`. Focused validator: 0 errors / 0 warnings; full corpus: 0 errors with only the five documented a1-01 entry-lesson warnings. Snapshot appended 49 immutable IDs after a green full-corpus run. Awaiting independent L2 QA; no L2 verdict recorded. |
