...
| `a1-04-donde-esta` | 2026-07-28 | L1-PASS | L2-FAIL(1) (3 CRIT / 4 MAJ / 5 MIN) | PENDING | PENDING | 1 | Where things and people are. Original content-pipeline draft: 15 vocab, 20 sentences, 6 passage segments, 4 quick checks, no reference card. Independent L2 at commit `215f19a`: **FAIL**. Three CRITICAL meaning-changing `acceptedEs` entries systematically replace explicit lexical subjects with pronouns/null subjects (s.11, s.12, s.17). Four MAJOR findings: spurious `pronoun.subject` tags on s.11/s.12; the taught `sobre`=`on` sense is systematically absent from sentence/passage EN accept sets; q.04's marked absence choice does not answer its positive `what is in` prompt. Five MINOR findings: one missing Spanish word-order alternate, vocab/recognition-side gaps, and combined max-size load (15 vocab + 20 sentences vs map ~10 vocab). Curriculum focus/register/leakage/explanation/hints/difficulty distribution otherwise pass. Focused validator was 0 errors / 0 warnings before QA; r1 corrections are author-validated and await targeted re-QA. See `review/qa/a1-04.qa1.json`. |
...
### a1-04 r1 fix loop — awaiting targeted re-QA
...
- Author-side focused validation is clean. The row remains `L2-FAIL(1)` until a separate reviewer runs targeted r2 QA; this note does not assert an L2 pass.
