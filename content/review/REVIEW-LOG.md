...
| `a1-04-donde-esta` | 2026-07-28 | L1-PASS | L2-PASS (0 CRIT / 0 MAJ / 0 MIN, r3) | PENDING | PENDING | 2 | Where things and people are. All 12 r1 content findings are closed: subject-preserving Spanish accept sets and honest tags, complete `sobre` recognition, corrected q.04 logic, all requested alternates, and a reduced active load of 13 vocab + 18 sentences with balanced 2/4/6/4/2 difficulty. Curriculum, register, leakage, Both-direction grading data, and assessment logic pass. Targeted r2 found one metadata-only blocker; `contentVersion: 2` corrects it, and independent r3 verified that the correction changed no authored payload or immutable ID. Focused validator: 0 errors / 0 warnings / 0 info; full validator: 0 errors with only the documented a1-01 findings; typecheck, contract smoke (13), and fixtures (18/18) pass. See `review/qa/a1-04.qa1.json`, `review/qa/a1-04.qa2.json`, and `review/qa/a1-04.qa3.json`. |
| `a1-05-ser-y-estar` | 2026-07-30 | L1-PASS | L2-FAIL (1 MAJ, r2) | PENDING | PENDING | 2 | Being: identity vs. state. R1's 7 local findings and 13 missing alternates are fixed; r2 is clean for those deltas. P-007 blocks L2-PASS because 23 required numbers remain ungraded under the 15-row schema cap. See `qa/a1-05.qa1.json` and `qa/a1-05.qa2.json`. |
...
### a1-04 r3 targeted re-QA — L2-PASS

- All 3 CRITICAL, 4 MAJOR, and 5 MINOR findings in `qa/a1-04.qa1.json` have corrective edits in the lesson artifact. Explicit lexical subjects are preserved in s.11/s.12/s.17, the spurious subject-pronoun tags are removed, recognition coverage and question logic are repaired, and active load is reduced to 13 vocab plus 18 sentences while every ID remains present.
- Independent r2 verified those repairs and found no linguistic regression. The sole metadata blocker was corrected with `contentVersion: 2`; independent metadata-only r3 confirmed the exact delta and closed L2 with no findings.

### a1-05 r2 targeted re-QA — local fixes clean; P-007 blocks L2-PASS

- R1 found 4 MAJOR and 4 MINOR issues. The author corrected all locally actionable findings: 13
  missing mixed-contraction English answers, two invalid s.15 alternates, six difficulty ratings,
  passage/quiz naturalness, explanation example count, and passage age answers. `contentVersion` is 2.
- Read-only r2 verified all seven local fixes and all 13 alternates with no focused regression.
  The focused validator is 0 errors / 0 warnings / 0 info.
- One MAJOR remains: the lesson teaches 0–30, but only 8 numbers have graded vocab records. The
  ungraded reference card cannot replace the Style Guide's one immutable VocabItem per set member.
  P-007 proposes the additive raw-cap widening; until approved and implemented, status stays L2-FAIL.
