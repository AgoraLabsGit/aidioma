...
| `a1-04-donde-esta` | 2026-07-28 | L1-PASS | L2-FAIL(2) (0 CRIT / 1 MAJ / 0 MIN) | PENDING | PENDING | 2 | Where things and people are. Targeted r2 confirms all 12 r1 content findings are closed: subject-preserving Spanish accept sets and honest tags, complete `sobre` recognition, corrected q.04 logic, all requested alternates, and a reduced active load of 13 vocab + 18 sentences with balanced 2/4/6/4/2 difficulty. Curriculum, register, leakage, Both-direction grading data, and assessment logic pass. The sole r2 metadata blocker is corrected by bumping `contentVersion` to 2; L2 remains failed pending a narrow independent metadata recheck. Focused validator: 0 errors / 0 warnings / 0 info; full validator: 0 errors with only the documented a1-01 findings; typecheck, contract smoke (13), and fixtures (18/18) pass. See `review/qa/a1-04.qa1.json` and `review/qa/a1-04.qa2.json`. |
...
### a1-04 r2 targeted re-QA — metadata correction applied
...
- Independent r2 verified those repairs and found no linguistic regression. The sole metadata blocker is corrected with `contentVersion: 2`; L2 remains failed pending a narrow independent metadata recheck.
