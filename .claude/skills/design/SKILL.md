...
6. Work → `done` + `done_summary` (or leave `active` + `open_questions` if Adv-blocked). Activity
   close: `decide`/`spec` complete with `ref: D-/SPEC-` when locked; else `design` complete/blocked
   with `ref: S-nnn`. Report S-id + D-id / SPEC-id.
