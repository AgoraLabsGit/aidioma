...
| PM-015 | Frequency / custom practice decks (words 1–100, 101–200…; user lists) | Saved practice is MVP; pre-populated frequency bands later | After saved_items + Review proven |
...
| PM-007 | Reading modality ES→EN (roadmap A7) | Reuses evaluate; fast-follow by design | After MVP ship |
| PM-008 | Constrained conversation mode: authored seed/persona/register/goals; evaluated turns; end recap; history + vocab capture | Highest AI cost; must never be unstructured chat without learning goals | Usage proves typed loop retained |
| PM-009 | KB growth: repeat-verdict cache → AI-correct answer becomes a **review candidate** → approved alternate; custom-topic generation passes provenance + validate/QA before becoming authored | Direct automatic promotion risks poisoning content; needs measured repeat misses/spend | AI spend or repeat-miss data justifies |
...
| PM-018 | Listen/pronunciation audio (browser speech synthesis first; authored/audioUrl later) | Audio is reserved but not a launch modality | After typed + flashcard loop is stable |
...
| PM-021 | Rich reading library: tap-to-gloss, genre/difficulty filters, reading-time/comprehension, save vocab | A7 launches the reusable reading evaluation first | Reading modality is proven and has repeat users |
